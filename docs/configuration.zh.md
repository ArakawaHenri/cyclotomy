...
检查点限额只约束新检查点和导入的历史；降低限额不会使已有检查点变得不可读取或无法恢复。工作区扫描失败时会跳过该检查点；当前位置得到保护后，后续轮次仍可创建检查点，无需重启 Cyclotomy。

改变 `storageDir` 是选择另一套存储，并不会移动现有数据。Pi 的 `PI_CODING_AGENT_DIR` 会同时改变 Agent 目录和 Cyclotomy 默认存储根目录的位置。

宿主识别出子代理与父会话共享工作区时，Cyclotomy 会自动跳过独立检查点管理。主会话、独立 worktree，以及宿主未提供这类信息的会话均按通常规则运行，无需额外配置。
...
自动清理回收不再被引用的对象。历史不会自动过期，也没有累计容量上限。清理在空闲时自动运行；前台操作需要工作区时会让路，之后自动重试。内存需求随保留历史的规模增长。调度文件只作提示：读取失败不会阻止清理，保存失败会与已完成的清理分开报告。
